...
export async function generateMetadata() {
  return buildPageMetadata("about", "es");
}
...
export default async function SpanishAboutPage() {
  ...
  return (
    <PageShell brand={brand} lang="es" orderPlatforms={content.orderPlatforms}>
      <InternalPageHero hero={pageHero(content, "about")} pageKey="about" lang="es" eyebrow="Sobre nosotros" />
      ...
      <section className="bg-cultured py-24">
        <div className="section-shell">
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-4">
            {values.map((value) => <article key={value} className="rounded-[1.5rem] bg-white p-7 shadow-soft"><h3 className="font-heading text-2xl">{value}</h3></article>)}
          </div>
        </div>
      </section>
    </PageShell>
